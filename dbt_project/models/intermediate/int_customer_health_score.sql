...
as_of as (
    select
        greatest(
            coalesce((select max(event_date) from {{ ref('stg_usage_events') }}), date '1900-01-01'),
            coalesce((select max(payment_date) from {{ ref('stg_payments') }}), date '1900-01-01'),
            coalesce((select max(created_date) from {{ ref('stg_support_tickets') }}), date '1900-01-01')
        ) as as_of_date
),
